...
-e "s/\(pxe_enabled\ =\ \).*/\1false/" $HOMEDIR/lc_startup.conf
    ...
    (echo y; echo bss; echo y; echo y) | $HOMEDIR/lc_startup.sh -i -c $HOMEDIR/lc_startup.conf
    setenable httpd
    setenable nginx
    setenable charge
...
-e "s/\(https:\/\/\)[0-9.]*\(\/;\)/\1$BSS_CONTROLLER\2/" /etc/nginx/conf.d/ssl.conf
...
-e "s/\(https:\/\/\)[0-9.]*\(\/;\)/\1$OSS_CONTROLLER\2/" /etc/nginx/conf.d/ssl.conf
...
-e "s/\(bss.uuid\ =\ \).*/\1$BSS_DOMAIN_ID/" /var/www/lcweb/lcc/config.ini
    sed -i -e 's/pack.enable = .*/pack.enable = 0;/' /var/www/lcweb/lcc/config.ini
...
'autotest@yunshan.net.cn',NULL,'2015-01-01 00:15:38','','0','4+qaY41WMy1kU2ueCApNp06gtl1JPKTFPouOwPt1Lis=','',
...
bss_sync_db()
{
    ...
    ${SSHPASS} -p ${OSS_PASS} ${SSH} root@${OSS_CONTROLLER} " mysql -e \"select lcuuid,name,type,charge_mode,state,price,content,plan_name,product_type,domain from livecloud.product_specification_v2_2 where domain='${OSS_DOMAIN_ID}';\" > /tmp/db_tmp "
    ${SSHPASS} -p ${OSS_PASS} ${SCP} root@${OSS_CONTROLLER}:/tmp/db_tmp /tmp/ 
    ${SED} -i '1d' /tmp/db_tmp
    ${MYSQL} -e "load data infile '/tmp/db_tmp' into table livecloud_bss.product_specification character set utf8 fields terminated by '\t' lines terminated by '\n' (lcuuid,name,type,charge_mode,state,price,content,plan_name,product_type,domain);"
    ${RM} -f /tmp/db_tmp
    ${SSHPASS} -p ${OSS_PASS} ${SSH} root@${OSS_CONTROLLER} "/bin/rm -f /tmp/db_tmp"
}

bss_mtps()
{
    str1="INSERT INTO \`pricing_plan\` VALUES 
(NULL,'2C2G','vm','2C2G','{\"disk\": {\"size\": \"0\"}, \"cpu\": {\"num\": \"2\"}, \"memory\": {\"size\": \"2\"}}','{\"disk\": \"1\", \"cpu\": \"2\", \"memory\": \"2\"}',
'"$OSS_DOMAIN_ID"','never',0,NULL),
(NULL,'4C4G','vm','4C4G','{\"disk\": {\"size\": \"0\"}, \"cpu\": {\"num\": \"4\"}, \"memory\": {\"size\": \"4\"}}','{\"disk\": \"1\", \"cpu\": \"4\", \"memory\": \"4\"}',
'"$OSS_DOMAIN_ID"','never',0,NULL);"
    ${MYSQL} -D livecloud_bss -e "$str1"
    
    str1="INSERT INTO \`unit_price\` VALUES 
(NULL,'cpu','cpu','cpu','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'memory','memory','memory','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'disk','disk','disk','{\"map\": [{\"domain\": \"[0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'vgateway','vgw','vgateway','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'valve','valve','valve','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'lb','load-balancer','lb','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'general_bandw','bandwidth','general_bandw','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'mobile_bandw','bandwidth','mobile_bandw','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'unicom_bandw','bandwidth','unicom_bandw','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'mobile_ip','ip','mobile_ip','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'unicom_ip','ip','unicom_ip','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'cloud_disk','block-device','cloud_disk','{\"map\": [{\"domain\": \"[0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'cloud_disk_snap','block-device-snapshot','cloud_disk_snap','{\"map\": [{\"domain\": \"[0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'snapshot','vm-snapshot','snapshot','{\"map\": [{\"domain\": \"(0, +Inf)\", \"b\": \"0\", \"k\": \"1\"}]}','per-day','"$OSS_DOMAIN_ID"'),
(NULL,'VFW','vfw','VFW','{\"map\": [{\"domain\": \"[0, +Inf)\", \"b\": \"0\", \"k\": \"1.5\"}]}','per-day','"$OSS_DOMAIN_ID"');"  
    /usr/bin/mysql -D livecloud_bss -e "$str1"
}
...
(NULL,'`uuidgen`','虚拟网关',1,4,1,0.000000000,'{\"description\": \"vgateway\", \"vgateway_info\": {\"wans\": 3, \"rate\": 1000, \"lans\": 3}}','vgateway',2,'"$OSS_DOMAIN_ID"'),
...
(NULL,'`uuidgen`','4C4G',1,4,1,0.000000000,'{\"user_disk_size\": 0, \"description\": \"4C4G\", \"compute_size\": {\"mem_size\": 4, \"vcpu_num\": 4, \"sys_disk_size\": 50}}','4C4G',1,'"$OSS_DOMAIN_ID"'),
(NULL,'`uuidgen`','2C2G',1,4,1,0.000000000,'{\"user_disk_size\": 0, \"description\": \"2C2G\", \"compute_size\": {\"mem_size\": 2, \"vcpu_num\": 2, \"sys_disk_size\": 50}}','2C2G',1,'"$OSS_DOMAIN_ID"'),
(NULL,'`uuidgen`','VFW',1,4,1,0.000000000,'{\"service_vendor\": \"TOPSEC\", \"user_disk_size\": 0, \"description\": \"VFW\", \"compute_size\": {\"mem_size\": 2, \"vcpu_num\": 2, \"sys_disk_size\": 50}}','VFW',16,'"$OSS_DOMAIN_ID"'),
...
(NULL,'`uuidgen`','虚拟机快照',2,4,1,0.000000000,'{\"description\": \"snapshot\"}','snapshot',11,'"$OSS_DOMAIN_ID"');"
    ...
    $MT pool.add-product-specification name=KVMPool product_specification=4C4G
    $MT pool.add-product-specification name=KVMPool product_specification=2C2G
    $MT pool.add-product-specification name=KVMPool product_specification=lb
    $MT pool.add-product-specification name=KVMPool product_specification=VFW
}
...
server_num=10 lease=12 activation_time=`date +"%Y-%m-%d"`)
    ...
    $MT vlantag-ranges.config type=ISP ranges=1-8
...
oss_add_IP_template()
{
    ...
    $MT storage.add backend=ceph-rbd domain=$OSS_DOMAIN name=capacity type=CAPACITY host=$host_list
    $MT storage.add backend=ceph-rbd domain=$OSS_DOMAIN name=performance type=CAPACITY host=$host_list
}
